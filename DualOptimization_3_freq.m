...
pulse_num=100;
F_DBS_=[10 20 130];%[5,10,20,30,50,100,130,200];
...
%     end
    ...
    for i=1:MaxIter_total
        [fitresult, gof] = gradient_fit_normalized_3param(F_DBS_, I_inf,x([1,3,4]), ub ,lb,x(2),T_syn,MaxIterGrad,'Normal');

        x_std=[fitresult.f,x(2),fitresult.F,fitresult.D];
    ...
    [fitresult, gof] = gradient_fit_normalized_3param(F_DBS_, I_inf,x([1,3,4]), ub ,lb,x0(2),T_syn,400,'Normal');
...
s=subplot(2,2,2)
hold on
x_values = linspace(min([U_mse,U_dual])*.9,max([U_mse,U_dual])*1.1,1000);
...
area(x_values,y_mse,'FaceColor',mse_color,'EdgeColor',mse_color,'FaceAlpha',0.5)
area(x_values,y_dual,'FaceColor',dual_color,'EdgeColor',dual_color,'FaceAlpha',0.8)
area([0 1],[NaN NaN],'FaceColor',std_color,'EdgeColor',std_color,'FaceAlpha',0.5)
